...
export default function MySubjects() {
    ...
    // Merge subject data with progress
    const mySubjects = mockSubjects.map(subject => {
        const progress = mockSubjectProgress.find(p => p.subject.toLowerCase() === subject.name.toLowerCase());
        return {
            ...subject,
            progress: progress?.progress || 0,
            completedLessons: progress?.completed || 0,
            totalLessons: progress?.lessons || 20,
            lastAccessed: '2 days ago', // Mock
            status: (progress?.progress || 0) > 0 ? 'in-progress' : 'not-started'
        };
    }).filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase()));

    const filteredSubjects = filter === 'all' 
        ? mySubjects 
        : mySubjects.filter(s => s.status === filter);
    ...
    return (
        <div className="p-6 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
            ...
            {mySubjects.some(s => s.progress > 0) && (
                <Card className="bg-gradient-to-r from-primary/10 via-primary/5 to-background border-primary/20">
                    <CardContent className="p-6 flex flex-col md:flex-row gap-6 items-center">
                        ...
                        <div className="flex-1 space-y-2 text-center md:text-left">
                            ...
                            <h2 className="text-2xl font-bold">Mathematics: Calculus II</h2>
                            <p className="text-muted-foreground">Unit 4: Integration Techniques • Lesson 3/8</p>
                            <div className="flex items-center gap-4 mt-2 justify-center md:justify-start">
                                <Progress value={65} className="w-32 md:w-64 h-2" />
